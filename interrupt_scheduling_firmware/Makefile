...
DRIVERS = crazyflie-firmware/drivers
FreeRTOS = crazyflie-firmware/lib/FreeRTOS
...
CFILES += $(COMMON)/$(BLINKY)/src/scheduling.c
CFILES += $(COMMON)/$(STM_LIB)/src/stm32f10x_dbgmcu.c

# STM chip specific flags
STFLAGS = -DSTM32F10X_MD -include $(COMMON)/$(BLINKY)/inc/stm32f10x_conf.h

# Define the compiler flags
CFLAGS = -O0 -g3 -g $(PROCESSOR) $(INCLUDE) $(STFLAGS) -Wl,--gc-sections -T $(COMMON)/$(BLINKY)/$(LINKER_SCRIPT)/stm32_flash.ld

# Build all relevant files and create .elf
all:
	@$(CC) $(CFILES) $(CFLAGS) $(COMMON)/$(CM3_STM)/$(STARTUP_STM)/startup_stm32f10x_md.s -o bin/blinky.elf lab04_tasks.a
